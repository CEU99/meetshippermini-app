-- =====================================================================
-- Diagnose Missing Match in Inbox - Supabase Compatible
-- =====================================================================
-- Use this to troubleshoot why a user doesn't see their match
-- =====================================================================

-- ⚙️ STEP 1: Find the user and their FID
-- =====================================================================
SELECT
  fid,
  username,
  display_name,
  '🔍 Step 1: User Lookup' as check
FROM users
...
-- Replace AYSU_FID_HERE with the actual FID number
-- =====================================================================

-- STEP 2: Find the match in question
-- =====================================================================
-- TODO: Replace AYSU_FID_HERE with actual FID from Step 1
SELECT
  m.id,
  m.user_a_fid,
  m.user_b_fid,
...
WHERE (m.user_a_fid = 543581 OR m.user_b_fid = 543581)
  AND (m.user_a_fid = AYSU_FID_HERE OR m.user_b_fid = AYSU_FID_HERE)
...
SELECT
  m.id,
  CASE
    WHEN m.user_a_fid = AYSU_FID_HERE THEN '👤 aysu16 is user_a'
    WHEN m.user_b_fid = AYSU_FID_HERE THEN '👤 aysu16 is user_b'
...
SELECT
  ...
  m.b_accepted,
  CASE
    WHEN m.user_a_fid = AYSU_FID_HERE AND m.a_accepted = false THEN '✅ Would show (user_a, not accepted)'
    WHEN m.user_b_fid = AYSU_FID_HERE AND m.b_accepted = false THEN '✅ Would show (user_b, not accepted)'
    ELSE '❌ Would NOT show'
  END as result,
...
WHERE (
  -- User is participant
  m.user_a_fid = AYSU_FID_HERE OR m.user_b_fid = AYSU_FID_HERE
)
AND (
...
SELECT
  ...
  CASE
    WHEN u.fid = AYSU_FID_HERE THEN '👤 aysu16'
    WHEN u.fid = 543581 THEN '👤 Emir'
    ELSE '👤 Other'
  END as who,
  '🔍 Step 7: User Records' as check
FROM users u
WHERE u.fid IN (AYSU_FID_HERE, 543581)
ORDER BY u.fid;
...
WHERE (m.user_a_fid = AYSU_FID_HERE OR m.user_b_fid = AYSU_FID_HERE)
  AND (
    (m.user_a_fid = AYSU_FID_HERE AND m.a_accepted = true AND m.b_accepted = false)
    OR
    (m.user_b_fid = AYSU_FID_HERE AND m.b_accepted = true AND m.a_accepted = false)
  )
