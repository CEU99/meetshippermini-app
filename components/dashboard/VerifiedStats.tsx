...
export default function VerifiedStats() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch verified stats');
  ...
  const containerVariants = {
    ...
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: 0.5, ease: 'easeOut' as any, staggerChildren: 0.1 },
    },
  };
  ...
  return (
    ...
    >
      ...
      {/* Stats */}
      {stats !== null && !isLoading && !error && (
        <div className="space-y-6">
          {/* Total Count */}
          <motion.div variants={itemVariants} className="text-center py-4 bg-white rounded-lg border border-green-100">
            <div className="text-4xl font-bold text-green-600 mb-1">{stats.totalCount}</div>
            <div className="text-sm text-gray-600">Total Verified</div>
          </motion.div>

          {/* Recent Users */}
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-3 px-1">Recently Verified</h4>
            <div className="space-y-2">
              {getDisplayUsers().length === 0 ? (
                <p className="text-center text-gray-500 text-sm">No recent verifications found.</p>
              ) : (
