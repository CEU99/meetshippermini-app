...
-- ============================================================================
-- 7. Test the fix (optional - comment out if you don't want to test)
-- ============================================================================

DO $$
DECLARE
  test_match_id UUID;
  test_user_a BIGINT := 11111;
  test_user_b BIGINT := 543581;
BEGIN
  RAISE NOTICE 'Testing the fix...';

  -- Find a test match between these users
  ...
  WHERE LEAST(user_a_fid, user_b_fid) = LEAST(test_user_a, test_user_b)
    ...
    AND status IN ('proposed', 'pending', 'accepted_by_a', 'accepted_by_b')
  LIMIT 1;
  ...
  IF test_match_id IS NOT NULL THEN
    ...
    BEGIN
      ...
      RAISE NOTICE '✅ Test decline succeeded for match %', test_match_id;

      -- Revert the test
      ROLLBACK;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE '❌ Test decline failed: %', SQLERRM;
      ROLLBACK;
    END;
  ELSE
    RAISE NOTICE '  No test match found between users % and %', test_user_a, test_user_b;
  END IF;
